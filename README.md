...
## Основные классы, их содержание и иерархия
### `Users`
Главный класс, определяющий пользователя. Может содержать всю заданную информацию - логин, токены, события.
Наследует класс `Calendar`, но он может не инициализировать родителя, если класс используется только для управления токенами.
##### Переменные
- _переменные класса `Calendar`_
- `name` - имя пользователя
- `tokens` - список токенов, ассоциированных с пользователем.
Может содержать неполную информацию (например, когда идёт запрос расписания от клиента, не нужно узнавать все токены - достаточно определить время последнего запроса и имя пользователя).

##### Методы
- `__construct($data, $type = 'token', $build_ical = true)` - конструктор, на вход принимает либо имя пользователя (например, когда нужно произвести операцию со списком токенов), либо токен (например, при запросе расписания),
а также указание на то, нужно ли будет сконструировать календарь;
- `get_username()` - находит имя пользователя в БД по первому в списке `tokens` токену и сохраняет его в `$this->username`. Как правило, нужен для конструирования iCal;
**- `request_lessons()` - запрашивает список событий у ЛК с именем пользователя, найденном в соответствующей переменной. Метод не доделан, но его надо сделать;**
- `get_full()` - находит информацию обо всех токенах пользователя, если логин неизвестен - обратится к `get_username()`;
- `create_token($description = '')` - создаёт новый токен с описанием `$description`, если у пользователя их менее четырёх;
- `delete_token($token_id)` - удаляет токен с номером `$token_id` у пользователя. Возвращает всю информацию об удалённом токене.

### `Token`
Класс, содержащий информацию о токене - логин/токен/время последнего обращения/описание/номер. По сути, он является шаблонным словарём, т. к. не имеет методов (кроме конструктора).


Класс, конструирующий iCal. Содержит общую конфигурацию параметров календаря - частота обновления, декларация часовых поясов, информация о версии генератора и синтаксиса, и т. д.
##### Переменные
Среди переменных содержит почти всё то же ("создано", "название", "интервал обновления", и т. д.), что и обычный iCal.
##### Методы
- `out()` - возвращает массив строк iCal:
- `ics()` - устанавливает нужные для клиента `;eaders`, выводит `out()`.

### `Lesson`
Класс, содержащий информацию об уроке (название предмета, группа, начало, конец, ссылка). Шаблонный словарь для удобного перевода принятой от ЛК информации в событии.

Остальные классы - `Standard`, `Timezone`, `Alarm`, `Event`, являются вспомогательными для составления iCal. Они не несут в себе информацию, которая обычно не меняется, но помогают её удобно выводить в виде массивов из строк.
